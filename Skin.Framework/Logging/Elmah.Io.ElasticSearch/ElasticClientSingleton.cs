...
namespace Skin.Framework.Logging.Elmah.Io.ElasticSearch
{
    /// <summary>
...
#pragma warning restore 618
            ...
            if (!string.IsNullOrWhiteSpace(esClusterConfig.Username) &&
                !string.IsNullOrWhiteSpace(esClusterConfig.Password))
            {
            ...
            var indexExistsResponse =
                esClient.IndexExists(new IndexExistsRequest(esClusterConfig.DefaultIndex)).VerifySuccessfulResponse();
        ...
        private static void CreateIndexWithMapping(IElasticClient esClient, string defaultIndex)
        {
            var descriptor = new CreateIndexDescriptor(defaultIndex)
                .Mappings(ms => ms
                    .Map<ErrorDocument>(m => m.AutoMap()
                        .Properties(CreateMultiFieldsForAllStrings))
                        //.Properties(ps => ps.Object<ProcessInformation>(n => n.AutoMap())
                    .Map<ProcessInformation>(m => m.AutoMap())
                );
            esClient.CreateIndex(descriptor).VerifySuccessfulResponse();
        }

        private static PropertiesDescriptor<ErrorDocument> CreateMultiFieldsForAllStrings(
            PropertiesDescriptor<ErrorDocument> props)
        {
        ...
        private static string LoadConnectionString(IDictionary config)
        {
            ...
            if (string.IsNullOrEmpty(connectionStringName))
            {
                throw new ApplicationException(
                    "You must specify the 'connectionStringName' attribute on the <errorLog /> element.");
            }
            ...
            throw new ApplicationException(string.Format("Could not find a ConnectionString with the name '{0}'.",
